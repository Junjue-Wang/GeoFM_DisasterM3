...
class ERModule(nn.Module, ConfigurableMixin):
    ...
    def init_from_weight_file(self):
        ...
        _logger.info(f'missing_keys ({len(IncompatibleKeys.missing_keys)}): {IncompatibleKeys.missing_keys}')
        _logger.info(f'unexpected_keys ({len(IncompatibleKeys.unexpected_keys)}): {IncompatibleKeys.unexpected_keys}')
