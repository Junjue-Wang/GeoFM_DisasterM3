...
def calculate_uncertainty(sem_seg_logits):
    ...
    Args:
        sem_seg_logits (Tensor): A tensor of shape (N, C, ...), where N is the minibatch size and
            C is the number of foreground classes. The values are logits.
    Returns:
        scores (Tensor): A tensor of shape (N, 1, ...) that contains uncertainty scores with
            the most uncertain locations having the highest uncertainty score.
    """
    if sem_seg_logits.size(1) > 1:
...
class PointRendSemSegHead(ERModule):
    def __init__(self, config):
        super(PointRendSemSegHead, self).__init__(config)

        in_channels = sum(self.config.in_channels_list)
        self.point_head = StandardPointHead(in_channels, **self.config.pointhead)

    def forward(self, features, coarse_sem_seg_logits, targets=None):
        if isinstance(features, torch.Tensor):
            features = [features]
        if self.training:
            return self._forward_train(features, coarse_sem_seg_logits, targets)
        else:
            return self._forward_test(features, coarse_sem_seg_logits)

    def _forward_train(self, features, coarse_sem_seg_logits, targets):
        losses = dict()
        with torch.no_grad():
        ...
        fine_grained_features = cat(
            [
                point_sample(feat, point_coords, align_corners=False)
        ...
        losses["pointrend_loss"] = F.cross_entropy(
            point_logits, point_targets, reduction="mean", ignore_index=self.config.ignore_index
        )
        return losses

    def _forward_test(self, features, coarse_sem_seg_logits):
        ...
        for _ in range(self.config.subdivision_steps):
            ...
            point_indices, point_coords = get_uncertain_point_coords_on_grid(
                uncertainty_map, self.config.subdivision_num_points
            )
            fine_grained_features = cat(
                [
                    point_sample(feat, point_coords, align_corners=False)
            ...
            sem_seg_logits = (
                sem_seg_logits.reshape(N, C, H * W)
                    .scatter_(2, point_indices, point_logits.float())
                    .view(N, C, H, W)
            )
        return sem_seg_logits
