...
class Launcher(object):
    def __init__(self,
        ...
        if self._master:
            self.init_model_dir()
            self._logger = Logger('EVER', use_tensorboard=self._master,
    ...
    def compute_loss_gradient(self, data):
        ...
        self.er_model.backward(losses, self.optimizer, self._amp, scaler=self.scaler)

        return msg_dict

    @staticmethod
    def log_info_dict(msg_dict):
        ...
        with torch.no_grad():
            if len(log_dict) != 0:
                ...
                for name, value in avg_log_dict.items():
                    if name not in output_info_dict:
                        output_info_dict[name] = 0.0
                    output_info_dict[name] += value.item() if isinstance(value,
